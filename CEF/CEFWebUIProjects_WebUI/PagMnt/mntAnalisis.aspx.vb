...
#End Region
        ...
        Private Sub cargaDgrMnt()
            Dim obrAnalisis As BusinessRules.Analisis = New BusinessRules.Analisis

            Dim dsAnalisis As DataSet = obrAnalisis.listar(1, 0)
            dgrdMnt.DataSource = dsAnalisis
            dgrdMnt.DataKeyField = "CodAnalisis"
            dgrdMnt.DataBind()
            obrAnalisis = Nothing

            lblNumReg.Text = dsAnalisis.Tables(0).Rows.Count.ToString()
        ...
        Private Sub imbNuevo_Click(ByVal sender As System.Object, ByVal e As System.Web.UI.ImageClickEventArgs) Handles imbNuevo.Click
            ...
            Dim dsAnalisis As DataSet = obrAnalisis.listar(1, 0)
            Dim drowDataRow As DataRow = dsAnalisis.Tables(0).NewRow()
            drowDataRow("Descripcion") = "(nuevo)"
            dsAnalisis.Tables(0).Rows.InsertAt(drowDataRow, 0)
            dgrdMnt.DataSource = dsAnalisis.Tables(0)
            dgrdMnt.DataKeyField = "CodAnalisis"
        ...
        Private Sub dgrdMnt_UpdateCommand(ByVal source As Object, ByVal e As System.Web.UI.WebControls.DataGridCommandEventArgs) Handles dgrdMnt.UpdateCommand
            If (hidAccion.Value = CType(Globals.ecMntPage.AGREGAR, String)) And (e.Item.ItemIndex = 0) Then
                ...
                Dim obeAnalisis As BusinessEntity.Analisis = New BusinessEntity.Analisis(txtDescripcion.Text, Int32.Parse(txtOrden.Text), Int32.Parse(dropEstado.SelectedValue))
                Dim voRC As Boolean = obrAnalisis.agregar(obeAnalisis)
                obrAnalisis = Nothing
            Else
                Dim intCodAnalisis As Integer = CType(dgrdMnt.DataKeys(e.Item.ItemIndex), Integer)
                Dim txtDescripcion As TextBox = CType(e.Item.FindControl("txtDescripcion"), TextBox)
                Dim txtOrden As TextBox = CType(e.Item.FindControl("txtOrden"), TextBox)
                Dim dropEstado As DropDownList = CType(e.Item.FindControl("dropEstado"), DropDownList)
                Dim obrAnalisis As BusinessRules.Analisis = New BusinessRules.Analisis

                Dim obeAnalisis As BusinessEntity.Analisis = New BusinessEntity.Analisis
                obeAnalisis.CodAnalisis = intCodAnalisis
                obeAnalisis.Descripcion = txtDescripcion.Text
                obeAnalisis.Orden = txtOrden.Text
                obeAnalisis.Estado = Int32.Parse(dropEstado.SelectedValue)
                Dim voRC As Boolean = obrAnalisis.modificar(obeAnalisis)
                obrAnalisis = Nothing
            End If
        ...
        Private Sub dgrdMnt_EditCommand(ByVal source As Object, ByVal e As System.Web.UI.WebControls.DataGridCommandEventArgs) Handles dgrdMnt.EditCommand
            ...
            Dim intCodAnalisis As Integer = CType(dgrdMnt.DataKeys(e.Item.ItemIndex), Integer)
            Dim obrAnalisis As BusinessRules.Analisis = New BusinessRules.Analisis
            Dim obeAnalisis As BusinessEntity.Analisis = obrAnalisis.leer(intCodAnalisis)

            dropEstado.DataSource = buscarGeneralTabla(Globals.ecTablaGeneral.ESTADO_ANALISIS)
            dropEstado.DataTextField = "Descripcion"
            dropEstado.DataValueField = "CodItem"
            dropEstado.DataBind()
            dropEstado.SelectedValue = obeAnalisis.Estado()
        End Sub
        ...
        Private Sub dgrdMnt_DeleteCommand(ByVal source As Object, ByVal e As System.Web.UI.WebControls.DataGridCommandEventArgs) Handles dgrdMnt.DeleteCommand
            ...
            Dim voRC As Boolean = obrAnalisis.eliminar(intCodAnalisis)
